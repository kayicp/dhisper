...
module {
  ...
  public func SHA256() : Digest = Digest(#SHA256);

  public func sha256(iters : { next() : ?{ next() : ?Nat8 } }) : Blob {
    let digest = SHA256();
    for (iter in iters) {
  ...
  public func sha256blobMap(iter : { next() : ?(Blob, Blob) }) : Blob {
    let digest = SHA256();
    for ((k, v) in iter) {
      digest.writeIter(k.vals());
      digest.writeIter(v.vals());
    };
    digest.sum();
  };

  public func SHA224() : Digest = Digest(#SHA224);
